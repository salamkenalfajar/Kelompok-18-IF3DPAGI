                ...
                <div class="h-full flex items-center justify-start gap-4 w-full px-4 md:px-0 mx-auto">
                    <form method="POST" action="{{ route('login') }}" class="gap-4 flex flex-col xl:pl-20">
                        ...
                        <label class="input input-bordered flex items-center gap-4 w-38">
                            ...
                            <input type="email" name="email" class="grow w-full" placeholder="Email" value="{{ old('email') }}" required />
                        </label>
                        @error('email')
                        <span class="text-red-500 text-center text-sm">{{ $message }}</span>
                        @enderror
                        <button type="submit" class="btn bg-color-coklat1 text-white w-full md:w-[250px] ">Daftar</button>
                    </form>
                </div>
